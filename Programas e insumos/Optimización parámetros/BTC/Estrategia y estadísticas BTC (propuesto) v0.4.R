...
# 12. VALORACIÓN, ESTADÍSTICAS Y OPTIMIZACIÓN DE ESTRATEGIAS ##################

# Cargue de tabla de niveles a optimizar para SL, TP y cierres parciales
TablaNivelesOpt <- read_excel(ArchivoInsumos, 
...
TablaNivelesOpt$Nivel1[match("Take Profit", TablaNivelesOpt$Variable)] <- quantile((BDPI$CLOSE/shift(BDPI$CLOSE, n=1, fill=NA) - 1), 0.7, na.rm = TRUE)
TablaNivelesOpt$Nivel2[match("Take Profit", TablaNivelesOpt$Variable)] <- quantile((BDPI$CLOSE/shift(BDPI$CLOSE, n=1, fill=NA) - 1), 0.8, na.rm = TRUE)
TablaNivelesOpt$Nivel3[match("Take Profit", TablaNivelesOpt$Variable)] <- quantile((BDPI$CLOSE/shift(BDPI$CLOSE, n=1, fill=NA) - 1), 0.9, na.rm = TRUE)
TablaNivelesOpt$Nivel1[match("Stop Loss", TablaNivelesOpt$Variable)] <- quantile((BDPI$CLOSE/shift(BDPI$CLOSE, n=1, fill=NA) - 1), 0.1, na.rm = TRUE)
TablaNivelesOpt$Nivel2[match("Stop Loss", TablaNivelesOpt$Variable)] <- quantile((BDPI$CLOSE/shift(BDPI$CLOSE, n=1, fill=NA) - 1), 0.2, na.rm = TRUE)
TablaNivelesOpt$Nivel3[match("Stop Loss", TablaNivelesOpt$Variable)] <- quantile((BDPI$CLOSE/shift(BDPI$CLOSE, n=1, fill=NA) - 1), 0.3, na.rm = TRUE)

# Tabla de posibles combinaciones de niveles de parámetros para optimización
NVariablesOpt <- 4
NivelesVariables <- 3
NPosibilidades_Opt <- NivelesVariables ^ NVariablesOpt
...
# Función consolidada para DIF, valoración y estadísticas

FunDIFValEst <- function(ListaBD, TP, SL, CierreTP, CierreLL) {
  
  # 12.1. DECISIÓN DE INVERSIÓN FINAL CONSIDERANDO SL Y TP ##################
  
  # Zonas de referencia de pérdidas y ganancias (P&G) para el cierre de posiciones
  ...
  # Asignación de zonas variables de pérdidas y ganancias (P&G) para el cierre de posiciones
  Zona_PG_Cierre$MinimoRazon[match("Take Profit",Zona_PG_Cierre$Zona_PG)] <- TP
  Zona_PG_Cierre$MaximoRazon[match("Low Profit",Zona_PG_Cierre$Zona_PG)] <- TP
  Zona_PG_Cierre$MinimoRazon[match("Low Profit",Zona_PG_Cierre$Zona_PG)] <- 0
  Zona_PG_Cierre$MaximoRazon[match("Low Loss",Zona_PG_Cierre$Zona_PG)] <- 0
  Zona_PG_Cierre$MinimoRazon[match("Low Loss",Zona_PG_Cierre$Zona_PG)] <- SL
  Zona_PG_Cierre$MaximoRazon[match("Stop Loss",Zona_PG_Cierre$Zona_PG)] <- SL
  Zona_PG_Cierre$Cierre[match("Take Profit",Zona_PG_Cierre$Zona_PG)] <- CierreTP
  Zona_PG_Cierre$Cierre[match("Low Loss",Zona_PG_Cierre$Zona_PG)] <- CierreLL
  ...
  ListaBD <- lapply(ListaBD, Fun_PE_PC_SENALSIGNO) # Construye lista de BDs
  
  # Determinación de decisión de inversión final (DIF) incorporando TP y SL
...
NEstParamOpt <- which.max(Cal_TP_SL$R_MDD)
EstParamOpt <- Cal_TP_SL$EST_OPTIMA[NEstParamOpt]

# 14. RESULTADOS DE ESTRATEGIA CON PARÁMETROS DE NEGOCIACIÓN ÓPTIMOS ##########


# Obtención de insumos para funciones de estrategia con parámetros de negociación óptimos
ResEstrategias <- FunDIFValEst(BDList,
...
SenalesOpt <- ResEstrategias[[c("Senales")]]
BDListOpt <- ResEstrategias[[c("ListaBD")]]
ListaEstOpt <- ResEstrategias[[c("ListaEst")]]

# Funcion para exportacion de estadisticas y graficos de estrategia optima
FunExp <- function(SenalesOpt) {
...
# Funcion para exportacion de estadisticas y graficos de estrategia optima
FunEstParamOpt <- function(ListaEstOpt, SenalesOpt) {
  ...
  View(ListaEstOpt[[EstParamOpt]]$BDRDiario)
  ...
  View(ListaEstOpt[[EstParamOpt]]$BDRMensual)
  ...
  View(ListaEstOpt[[EstParamOpt]]$BDRAnual)
...
FunEstParamOpt(ListaEstOpt, SenalesOpt)

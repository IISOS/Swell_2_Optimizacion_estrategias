...
# Eliminación de FH descartadas para calcular vela diaria
...
# 12. CÁLCULO ESTADÍSTICAS RETORNO Y RIESGO ###################################
...
Fun_Est_U_MPA <- function(BD) {
  ...
  G_UTILIDADACUM <- ggplot(BD, aes(x = DATE, y = UTILIDADACUM)) +
                      geom_line() +
                      ggtitle("Utilidad acumulada por estrategia") + 
...
# 13. RESUMEN ESTADÍSTICAS ESTRATEGIAS ########################################
